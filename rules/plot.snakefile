...
rule ccs_stats_plot_cdf:
    ...
    run:
        ...
        # Get stat list
        size_list = ccsstatlib.plot.get_metric_list(
            wildcards.sample, metric='len', per_cell=wildcards.per_cell == 'cell'
        )

        # Get figure
        fig = ccsstatlib.plot.get_cdf_plot(size_list)
...
rule ccs_stats_plot_line_hist:
    ...
    run:
        ...
        # Get list of values
        stat_list = ccsstatlib.plot.get_metric_list(
            wildcards.sample, metric=wildcards.metric, per_cell=wildcards.per_cell == 'cell'
        )

        # Get figure
        fig = ccsstatlib.plot.get_hist_plot(stat_list, wildcards.metric)
...
rule ccs_stats_plot_read_len_density:
    input:
        tsv=lambda wildcards: [
            '{0}/cells/{1}/zmw_summary.tsv.gz'.format(SAMPLE_NAME, cell)
            for cell in get_cell_dict().keys()
    ...
    run:
        ...
        fig = ccsstatlib.plot.get_density_plot(
